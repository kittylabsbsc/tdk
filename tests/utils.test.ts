...
import { Decimal, Tuli } from '../src'
import { Blockchain, generatedWallets } from '@tulilabs/core/dist/utils'
import { JsonRpcProvider } from '@ethersproject/providers'
import { mintCurrency, setupTuli, TuliConfiguredAddresses } from './helpers'
  ...
  describe('EIP-712 Utilities', () => {
    describe('#signPermitMessage', () => {
      ...
      it('signs a permit message that is able to be processed on chain', async () => {
        const provider = new JsonRpcProvider()
        const [mainWallet, otherWallet] = generatedWallets(provider)
        const onChainConfig = await setupTuli(mainWallet, [otherWallet])
        const mainTuli = new Tuli(
          mainWallet,
          50,
          onChainConfig.media,
        ...
        await mainTuli.mint(mediaData, bidShares)
    ...
    describe('#signMintWithSig', () => {
      ...
      it('signs a mintWithSig message that is able to be processed on chain', async () => {
        ...
        await otherTuli.mintWithSig(mainWallet.address, mediaData, bidShares, eipSig)
